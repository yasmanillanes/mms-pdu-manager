...
import com.google.mms.ContentType;
...
public class PduParser {
	 ...
	 * @param parseContentDisposition whether to parse the Content-Disposition part
	 *                                header
	 */
	...
	public GenericPdu parse() {
		...
		case PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF:
			...
			if (ctTypeStr.equals(ContentType.MULTIPART_MIXED) || ctTypeStr.equals(ContentType.MULTIPART_RELATED)
				...
				System.out.println("Unsupported ContentType: " + ctTypeStr);
			}
			return null;
		case PduHeaders.MESSAGE_TYPE_DELIVERY_IND:
			if (LOCAL_LOGV) {
				System.out.println("parse: MESSAGE_TYPE_DELIVERY_IND");
			}
			DeliveryInd deliveryInd = new DeliveryInd(mHeaders);
			return deliveryInd;
		case PduHeaders.MESSAGE_TYPE_ACKNOWLEDGE_IND:
			if (LOCAL_LOGV) {
				System.out.println("parse: MESSAGE_TYPE_ACKNOWLEDGE_IND");
			}
			AcknowledgeInd acknowledgeInd = new AcknowledgeInd(mHeaders);
			return acknowledgeInd;
		case PduHeaders.MESSAGE_TYPE_READ_ORIG_IND:
			if (LOCAL_LOGV) {
				System.out.println("parse: MESSAGE_TYPE_READ_ORIG_IND");
			}
			ReadOrigInd readOrigInd = new ReadOrigInd(mHeaders);
			return readOrigInd;
		case PduHeaders.MESSAGE_TYPE_READ_REC_IND:
			if (LOCAL_LOGV) {
				System.out.println("parse: MESSAGE_TYPE_READ_REC_IND");
			}
	...
	protected PduHeaders parseHeaders(ByteArrayInputStream pduDataStream) {
		...
		while (keepParsing && (pduDataStream.available() > 0)) {
			...
			if ((headerField >= TEXT_MIN) && (headerField <= TEXT_MAX)) {
				...
				if (LOCAL_LOGV) {
					System.out.println("TextHeader: " + new String(bVal));
				}
			...
			case PduHeaders.MESSAGE_TYPE: {
				...
				if (LOCAL_LOGV) {
					System.out.println("parseHeaders: messageType: " + messageType);
				}
				switch (messageType) {
			...
			case PduHeaders.STORE_STATUS:
				/**
				 * The following field has a different value when used in the M-Mbox-Delete.conf
				 * and M-Delete.conf PDU. For now we ignore this fact, since we do not support
				 * these PDUs
				 */
			case PduHeaders.RESPONSE_STATUS: {
				int value = extractByteValue(pduDataStream);
				if (LOCAL_LOGV) {
					System.out.println("parseHeaders: byte: " + headerField + " value: " + value);
				}
			...
			case PduHeaders.MESSAGE_SIZE: {
				try {
					...
					if (LOCAL_LOGV) {
						System.out.println("parseHeaders: longint: " + headerField + " value: " + value);
					}
			...
			case PduHeaders.LIMIT: {
				try {
					...
					if (LOCAL_LOGV) {
						System.out.println("parseHeaders: int: " + headerField + " value: " + value);
					}
					headers.setLongInteger(value, headerField);
				} catch (RuntimeException e) {
					log(headerField + "is not Long-Integer header field!");
			...
			case PduHeaders.REPLY_APPLIC_ID:
				/**
				 * The next three header fields are email addresses as defined in RFC2822, not
			...
			case PduHeaders.CONTENT_LOCATION: {
				...
				if (null != value) {
					try {
						if (LOCAL_LOGV) {
							System.out.println("parseHeaders: string: " + headerField + " value: " + new String(value));
						}
			...
			case PduHeaders.RESPONSE_TEXT: {
				...
				if (null != value) {
					try {
						if (LOCAL_LOGV) {
							System.out.println(
									"parseHeaders: encoded string: " + headerField + " value: " + value.getString());
						}
			...
			case PduHeaders.TO: {
				...
				if (null != value) {
					...
					if (null != address) {
						...
						if (LOCAL_LOGV) {
							System.out.println("parseHeaders: (to/cc/bcc) address: " + headerField + " value: " + str);
						}
			...
			case PduHeaders.REPLY_CHARGING_DEADLINE: {
				...
				try {
					if (LOCAL_LOGV) {
						System.out.println("parseHeaders: time value: " + headerField + " value: " + timeValue);
					}
			...
			case PduHeaders.FROM: {
				...
				try {
					if (LOCAL_LOGV) {
						System.out
								.println("parseHeaders: from address: " + headerField + " value: " + from.getString());
					}
			...
			case PduHeaders.MESSAGE_CLASS: {
				...
				if (LOCAL_LOGV) {
					System.out.println("parseHeaders: MESSAGE_CLASS: " + headerField + " value: " + messageClass);
				}
				...
				if (messageClass >= PduHeaders.MESSAGE_CLASS_PERSONAL) {
					...
					try {
						if (PduHeaders.MESSAGE_CLASS_PERSONAL == messageClass) {
							headers.setTextString(PduHeaders.MESSAGE_CLASS_PERSONAL_STR.getBytes(),
									PduHeaders.MESSAGE_CLASS);
						} else if (PduHeaders.MESSAGE_CLASS_ADVERTISEMENT == messageClass) {
							headers.setTextString(PduHeaders.MESSAGE_CLASS_ADVERTISEMENT_STR.getBytes(),
									PduHeaders.MESSAGE_CLASS);
						} else if (PduHeaders.MESSAGE_CLASS_INFORMATIONAL == messageClass) {
							headers.setTextString(PduHeaders.MESSAGE_CLASS_INFORMATIONAL_STR.getBytes(),
									PduHeaders.MESSAGE_CLASS);
			...
			case PduHeaders.MMS_VERSION: {
				...
				try {
					if (LOCAL_LOGV) {
						System.out.println("parseHeaders: MMS_VERSION: " + headerField + " value: " + version);
					}
			...
			case PduHeaders.PREVIOUSLY_SENT_BY: {
				...
				if (null != previouslySentBy) {
					try {
						if (LOCAL_LOGV) {
							System.out.println("parseHeaders: PREVIOUSLY_SENT_BY: " + headerField + " value: "
			...
			case PduHeaders.MM_FLAGS: {
				 ...
				 */
				if (LOCAL_LOGV) {
					System.out.println("parseHeaders: MM_FLAGS: " + headerField + " NOT REALLY SUPPORTED");
				}
				...
				/*
				 * not store this header filed in "headers", because now PduHeaders doesn't
				 * support it
				 */
				break;
			}
			...
			case PduHeaders.MBOX_QUOTAS: {
				if (LOCAL_LOGV) {
					System.out.println("parseHeaders: MBOX_TOTALS: " + headerField);
				}
				/* Value-length */
				parseValueLength(pduDataStream);
				...
				/*
				 * not store these headers filed in "headers", because now PduHeaders doesn't
				 * support them
				 */
				break;
			}
			...
			case PduHeaders.ELEMENT_DESCRIPTOR: {
				if (LOCAL_LOGV) {
					System.out.println("parseHeaders: ELEMENT_DESCRIPTOR: " + headerField);
				}
			...
			case PduHeaders.CONTENT_TYPE: {
				...
				if (null != contentType) {
					try {
						if (LOCAL_LOGV) {
							System.out.println("parseHeaders: CONTENT_TYPE: " + headerField + contentType.toString());
						}
			...
			default: {
				if (LOCAL_LOGV) {
					System.out.println("parseHeaders: Unknown header: " + headerField);
				}
				log("Unknown header");
			}
			}
	...
	protected PduBody parseParts(ByteArrayInputStream pduDataStream) {
		...
		for (int i = 0; i < count; i++) {
			...
			/*
			 * FIXME: check content-id, name, filename and content location, if not set
			 * anyone of them, generate a default content-location
			 */
	...
	protected static int parseUnsignedInt(ByteArrayInputStream pduDataStream) {
		/**
		 * From wap-230-wsp-20010705-a.pdf The maximum size of a uintvar is 32 bits. So
		 * it will be encoded in no more than 5 octets.
		 */
	...
	protected static int parseValueLength(ByteArrayInputStream pduDataStream) {
		/**
		 * From wap-230-wsp-20010705-a.pdf Value-length = Short-length | (Length-quote
		 * Length) Short-length = <Any octet 0-30> Length-quote = <Octet 31> Length =
		 * Uintvar-integer Uintvar-integer = 1*5 OCTET
		 */
		assert (null != pduDataStream);
		int temp = pduDataStream.read();
		assert (-1 != temp);
	...
	protected static EncodedStringValue parseEncodedStringValue(ByteArrayInputStream pduDataStream) {
		/**
		 * From OMA-TS-MMS-ENC-V1_3-20050927-C.pdf Encoded-string-value = Text-string |
	...
	protected static byte[] parseWapString(ByteArrayInputStream pduDataStream, int stringType) {
		assert (null != pduDataStream);
		/**
		 * From wap-230-wsp-20010705-a.pdf Text-string = [Quote] *TEXT End-of-string If
		 * the first character in the TEXT is in the range of 128-255, a Quote character
		 * must precede it. Otherwise the Quote character must be omitted. The Quote is
		 * not part of the contents. Quote = <Octet 127> End-of-string = <Octet 0>
		 *
	...
	protected static boolean isTokenCharacter(int ch) {
		/**
		 * Token = 1*<any CHAR except CTLs or separators> separators = "("(40) | ")"(41)
		 * | "<"(60) | ">"(62) | "@"(64) | ","(44) | ";"(59) | ":"(58) | "\"(92) |
		 * <">(34) | "/"(47) | "["(91) | "]"(93) | "?"(63) | "="(61) | "{"(123) |
		 * "}"(125) | SP(32) | HT(9) CHAR = <any US-ASCII character (octets 0 - 127)>
		 * CTL = <any US-ASCII control character (octets 0 - 31) and DEL (127)> SP =
	...
	protected static boolean isText(int ch) {
		/**
		 * TEXT = <any OCTET except CTLs, but including LWS> CTL = <any US-ASCII control
		 * character (octets 0 - 31) and DEL (127)> LWS = [CRLF] 1*( SP | HT ) CRLF = CR
		 * LF CR = <US-ASCII CR, carriage return (13)> LF = <US-ASCII LF, linefeed (10)>
		 */
	...
	protected static int parseShortInteger(ByteArrayInputStream pduDataStream) {
		/**
		 * From wap-230-wsp-20010705-a.pdf Short-integer = OCTET Integers in range 0-127
		 * shall be encoded as a one octet value with the most significant bit set to
		 * one (1xxx xxxx) and with the value in the remaining least significant bits.
		 */
		assert (null != pduDataStream);
		int temp = pduDataStream.read();
		assert (-1 != temp);
	...
	protected static long parseLongInteger(ByteArrayInputStream pduDataStream) {
		 ...
		 * Multi-octet-integer Multi-octet-integer = 1*30 OCTET The content octets shall
		 * be an unsigned integer value with the most significant octet encoded first
		 * (big-endian representation). The minimum number of octets must be used to
		 * encode the value. Short-length = <Any octet 0-30>
		 */
		assert (null != pduDataStream);
		int temp = pduDataStream.read();
		assert (-1 != temp);
	 ...
	 */
	protected static void parseContentTypeParams(ByteArrayInputStream pduDataStream, HashMap<Integer, Object> map,
			Integer length) {
		/**
		 * From wap-230-wsp-20010705-a.pdf Parameter = Typed-parameter |
		 * Untyped-parameter Typed-parameter = Well-known-parameter-token Typed-value
		 * the actual expected type of the value is implied by the well-known parameter
		 * Well-known-parameter-token = Integer-value the code values used for
		 * parameters are specified in the Assigned Numbers appendix Typed-value =
		 * Compact-value | Text-value In addition to the expected type, there may be no
		 * value. If the value cannot be encoded using the expected type, it shall be
		 * encoded as text. Compact-value = Integer-value | Date-value |
		 * Delta-seconds-value | Q-value | Version-value | Uri-value Untyped-parameter =
		 * Token-text Untyped-value the type of the value is unknown, but it shall be
		 * encoded as an integer, if that is possible. Untyped-value = Integer-value |
		 * Text-value
		 */
		assert (null != pduDataStream);
		assert (length > 0);
		...
		while (0 < lastLen) {
			...
			switch (param) {
			/**
			 * From rfc2387, chapter 3.1 The type parameter must be specified and its value
			 * is the MIME media type of the "root" body part. It permits a MIME user agent
			 * to determine the content-type without reference to the enclosed body part. If
			 * the value of the type parameter and the root body part's content-type differ
			...
			/**
			 * From oma-ts-mms-conf-v1_3.pdf, chapter 10.2.3. Start Parameter Referring to
			 * Presentation
			 *
			 * From rfc2387, chapter 3.2 The start parameter, if given, is the content-ID of
			 * the compound object's "root". If not present the "root" is the first body
			 * part in the Multipart/Related entity. The "root" is the element the
			...
			/**
			 * From oma-ts-mms-conf-v1_3.pdf In creation, the character set SHALL be either
			 * us-ascii (IANA MIBenum 3) or utf-8 (IANA MIBenum 106)[Unicode]. In retrieval,
			 * both us-ascii and utf-8 SHALL be supported.
			 *
			 * From wap-230-wsp-20010705-a.pdf charset = Well-known-charset|Text-String
			 * Well-known-charset = Any-charset | Integer-value Both are encoded using
			 * values from Character Set Assignments table in Assigned Numbers Any-charset =
			 * <Octet 128> Equivalent to the special RFC2616 charset value "*"
			 */
			case PduPart.P_CHARSET:
				...
				if (((firstValue > TEXT_MIN) && (firstValue < TEXT_MAX)) || (END_STRING_FLAG == firstValue)) {
					...
					try {
						...
						System.err.println(Arrays.toString(charsetStr));
						e.printStackTrace();
			...
			/**
			 * From oma-ts-mms-conf-v1_3.pdf A name for multipart object SHALL be encoded
			 * using name-parameter for Content-Type header in WSP multipart headers.
			 *
			...
			default:
				if (LOCAL_LOGV) {
					System.out.println("Not supported Content-Type parameter");
				}
				if (-1 == skipWapValue(pduDataStream, lastLen)) {
					System.err.println("Corrupt Content-Type");
				} else {
					lastLen = 0;
				}
				break;
	...
	protected static byte[] parseContentType(ByteArrayInputStream pduDataStream, HashMap<Integer, Object> map) {
		...
		if (cur < TEXT_MIN) {
			...
			if (parameterLen < 0) {
				System.err.println("Corrupt MMS message");
				return (PduContentTypes.contentTypes[0]).getBytes(); // "*/*"
			}
	...
	protected boolean parsePartHeaders(ByteArrayInputStream pduDataStream, PduPart part, int length) {
		 ...
		 * multipart headers. In decoding, name-parameter of Content-Type SHALL be used
		 * if available. If name-parameter of Content-Type is not available, filename
		 * parameter of Content-Disposition header SHALL be used if available. If
		 * neither name-parameter of Content-Type header nor filename parameter of
		 * Content-Disposition header is available, Content-Location header SHALL be
		 * used if available.
		 *
		 * Within SMIL part the reference to the media object parts SHALL use either
		 * Content-ID or Content-Location mechanism [RFC2557] and the corresponding WSP
		 * part headers in media object parts contain the corresponding definitions.
		 */
		int startPos = pduDataStream.available();
		int tempPos = 0;
		int lastLen = length;
		while (0 < lastLen) {
			...
			if (header > TEXT_MAX) {
				...
				case PduPart.P_CONTENT_LOCATION:
					/**
					 * From wap-230-wsp-20010705-a.pdf, chapter 8.4.2.21 Content-location-value =
					 * Uri-value
					 */
				...
				case PduPart.P_CONTENT_DISPOSITION:
					/**
					 * From wap-230-wsp-20010705-a.pdf, chapter 8.4.2.21 Content-disposition-value =
					 * Value-length Disposition *(Parameter) Disposition = Form-data | Attachment |
					 * Inline | Token-text Form-data = <Octet 128> Attachment = <Octet 129> Inline =
					 * <Octet 130>
					 */

					/*
					 * some carrier mmsc servers do not support content_disposition field correctly
					 */
				...
				default:
					if (LOCAL_LOGV) {
						System.out.println("Not supported Part headers: " + header);
					}
					if (-1 == skipWapValue(pduDataStream, lastLen)) {
						System.err.println("Corrupt Part headers");
						return false;
					}
					lastLen = 0;
					break;
	 ...
	 * @return part position, THE_FIRST_PART when it's the first one, THE_LAST_PART

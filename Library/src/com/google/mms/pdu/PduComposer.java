...
public class PduComposer {
	...
	static final String REGEXP_IPV6_ADDRESS_TYPE = "[a-fA-F]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}"
			+ "[a-fA-F0-9]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}" + "[a-fA-F0-9]{4}\\:{1}[a-fA-F0-9]{4}";
	...
	/**
	 * Make the message. No need to check whether mandatory fields are set, because
	 * the constructors of outgoing pdus are taking care of this.
	 *
	...
	/**
	 * Append short integer value to mMessage. This implementation doesn't check the
	 * validity of parameter, since it assumes that the values are validated in the
	 * GenericPdu setter methods.
	 */
	protected void appendShortInteger(int value) {
		/*
		 * From WAP-230-WSP-20010705-a: Short-integer = OCTET ; Integers in range 0-127
		 ...
		 * In our implementation, only low 7 bits are stored and otherwise bits are
		 * ignored.
		 */
	 ...
	 * implementation doesn't check the validity of parameter, since it assumes that
	 * the values are validated in the GenericPdu setter methods.
	 */
	...
	/**
	 * Append a short length into mMessage. This implementation doesn't check the
	 * validity of parameter, since it assumes that the values are validated in the
	 * GenericPdu setter methods.
	 */
	...
	/**
	 * Append long integer into mMessage. it's used for really long integers. This
	 * implementation doesn't check the validity of parameter, since it assumes that
	 * the values are validated in the GenericPdu setter methods.
	 */
	protected void appendLongInteger(long longInt) {
		/*
		 * From WAP-230-WSP-20010705-a: Long-integer = Short-length Multi-octet-integer
		 * ; The Short-length indicates the length of the Multi-octet-integer
		 * Multi-octet-integer = 1*30 OCTET ; The content octets shall be an unsigned
		 * integer value with the ; most significant octet encoded first (big-endian
		 * representation). ; The minimum number of octets must be used to encode the
		 * value.
		 */
		int size;
		long temp = longInt;
	...
	protected void appendTextString(byte[] text) {
		/*
		 * From WAP-230-WSP-20010705-a: Text-string = [Quote] *TEXT End-of-string ; If
		 * the first character in the TEXT is in the range of 128-255, ; a Quote
	...
	/**
	 * Append encoded string value to mMessage. This implementation doesn't check
	 * the validity of parameter, since it assumes that the values are validated in
	 * the GenericPdu setter methods.
	 */
	protected void appendEncodedString(EncodedStringValue enStr) {
		/*
		 * From OMA-TS-MMS-ENC-V1_3-20050927-C: Encoded-string-value = Text-string |
		...
		/*
		 * In the implementation of EncodedStringValue, the charset field will never be
		 * 0. It will always be composed as Encoded-string-value = Value-length Char-set
		 * Text-string
		 */
		mStack.newbuf();
	...
	/**
	 * Append uintvar integer into mMessage. This implementation doesn't check the
	 * validity of parameter, since it assumes that the values are validated in the
	 * GenericPdu setter methods.
	 */
	protected void appendUintvarInteger(long value) {
		/*
		 * From WAP-230-WSP-20010705-a: To encode a large unsigned integer, split it
		 * into 7-bit fragments and place them in the payloads of multiple octets. The
		 * most significant bits are placed in the first octets with the least
		 * significant bits ending up in the last octet. All octets MUST set the
		 * Continue bit to 1 except the last octet, which MUST set the Continue bit to
		 * 0.
		 */
		int i;
	...
	protected void appendValueLength(long value) {
		/*
		 * From WAP-230-WSP-20010705-a: Value-length = Short-length | (Length-quote
		 * Length) ; Value length is used to indicate the length of the value to follow
		 * Short-length = <Any octet 0-30> Length-quote = <Octet 31> Length =
		 * Uintvar-integer
		 */
	...
	protected void appendQuotedString(byte[] text) {
		/*
		 * From WAP-230-WSP-20010705-a: Quoted-string = <Octet 34> *TEXT End-of-string
	...
	private int appendHeader(int field) {
		...
		case PduHeaders.FROM:
			...
			if ((from == null) || "".equals(from.getString())
	...
	private int makeMessageBody() {
		...
		for (int i = 0; i < partNum; i++) {
			...
			/*
			 * Content-type parameter : name. The value of name, filename, content-location
			 * is the same. Just one of them is enough for this PDU.
			 */
			...
			if (null == name) {
				...
				if (null == name) {
					...
					if (null == name) {
						/*
						 * at lease one of name, filename, Content-location should be available.
						 */
						return PDU_COMPOSE_CONTENT_ERROR;
					}
				}
	...
	/**
	 * This implementation can be OPTIMIZED to use only 2 buffers. This optimization
	 * involves changing BufferStack only... Its usage (interface) will not change.
	 */
	 ...
	 * Check address type.
	 *
	 * @param address address string without the postfix stinng type, such as
	 *                "/TYPE=PLMN", "/TYPE=IPv6" and "/TYPE=IPv4"
	 * @return PDU_PHONE_NUMBER_ADDRESS_TYPE if it is phone number,
	 *         PDU_EMAIL_ADDRESS_TYPE if it is email address, PDU_IPV4_ADDRESS_TYPE
	 *         if it is ipv4 address, PDU_IPV6_ADDRESS_TYPE if it is ipv6 address,
	 *         PDU_UNKNOWN_ADDRESS_TYPE if it is unknown.
	 */
	protected static int checkAddressType(String address) {
		/**
		 * From OMA-TS-MMS-ENC-V1_3-20050927-C.pdf, section 8. address = ( e-mail /
		 * device-address / alphanum-shortcode / num-shortcode) e-mail = mailbox; to the
		 * definition of mailbox as described in section 3.4 of [RFC2822], but excluding
		 * the obsolete definitions as indicated by the "obs-" prefix. device-address =
		 * ( global-phone-number "/TYPE=PLMN" ) / ( ipv4 "/TYPE=IPv4" ) / ( ipv6
		 * "/TYPE=IPv6" ) / ( escaped-value "/TYPE=" address-type )
		 *
